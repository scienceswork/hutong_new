...
@section('seo_title')大V @if($experts->currentPage()>1))- 第{{ $experts->currentPage() }}页 @endif - {{ Setting()->get('website_name') }}@endsection
    ...
    <div class="row">
        <div class="col-xs-12 col-md-9 main">
            <div class="widget-streams users">
                        ...
                        <div class="company-func-detail">
                            <form name="searchForm" action="{{ route('website.experts') }}" method="GET">
                                ...
                                <div class="col-md-2 search-btn-warp">
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-primary search-btn">搜索大V</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <h2 class="h4  mt-30">大列表V<a href="{{ route('auth.authentication.index') }}" class="pull-right text-danger" target="_blank">申请成为大V <i class="fa fa-external-link-square text-green" aria-hidden="true"></i></a></h2>
